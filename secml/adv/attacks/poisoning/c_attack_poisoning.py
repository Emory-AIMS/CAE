...
class CAttackPoisoning(CAttack):
    ...
    def _rnd_init_poisoning_points(
        ...
        if self.init_type == 'mean_outlier':
            samples = init_dataset.X.deepcopy()
            samples = samples.tondarray()
            meanx = np.transpose(np.average(samples, axis=0))
            diff = np.linalg.norm((samples - meanx), axis=1)
            sorted_index = sorted(range(len(diff)), key=lambda k: diff[k])
            idx = np.array(sorted_index[-n_points:])
            idx = CArray(idx)
        elif self.init_type == 'random':
            idx = CArray.randsample(init_dataset.num_samples, n_points, random_state=self.random_seed)
        else: 
        ...
        ### comment these lines for just optimizing without flipping
    ...
    def _objective_function_gradient(self, xc, normalization=True):
        ...
        y_ts = self._y_target if self._y_target is not None else self.val.Y
        # y_ts_np = CArray(y_ts).tondarray()
        # y_val_np = CArray(self.val.Y).tondarray()
        # print("y_ts_np:")
        # print(y_ts_np)
        # print("y_val_np:")
        # print(y_val_np)
...
#             # randomly sample xc and yc
        xc, yc, idx_c = self._rnd_init_poisoning_points(init_from_val=self.init_from_val)
            # print(CArray(idx).tondarray())
...
#         while delta > 0 and k < max_iter:
        ...
        while k < max_iter:
            ...
            for i in range(self._n_points):
                ...
                ### for just optimizing without flipping: comment ending lines at _rnd_init_poisoning_points
                ### for just flipping without optiminzing comment below line and uncomment the line after it with no xc[idx,:] at the left side
                xc[idx, :] = self._run(xc, yc, idx=idx)
                # self._run(xc, yc, idx=idx) ### for just flipping
...
#                     xc[idx, :] = xc[idx-1, :]
#                     yc[idx] = yc[idx-1]
#                     y_pred, scores = self._poisoned_clf.predict(x, return_decision_function=True)
#                     acc = metric.performance_score(y_true=y, y_pred=y_pred)
                
                print('Point {} generated!'.format(i))
                prev_acc = acc
        ...
        return y_pred, scores, CDataset(xc, yc), self._f_opt, idx_c
